  ...
  return (
    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
      {stages.map(s => {
        ...
        const cls = !s.data ? 'background: var(--border2)' : hasAlert ? 'background: var(--red)' : 'background: var(--green)'
        return (
          <div
            key={s.key}
            title={`${s.label}: ${!s.data ? 'Pendiente' : hasAlert ? 'Con alerta' : 'OK'}`}
            style={{ width: 8, height: 8, borderRadius: '50%', ...(!s.data ? { background: 'var(--border2)' } : hasAlert ? { background: 'var(--red)' } : { background: 'var(--green)' }) }}
          />
        )
      })}
    </div>
